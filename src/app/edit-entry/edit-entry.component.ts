...
export class EditEntryComponent implements OnInit {
  ...
  constructor(
    private entryService: ExpenseEntryService,
  ...
  ngOnInit() {
    ...
    if (this.selectedId != null && this.selectedId != 0) {
      this.entryService.getExpenseEntry(this.selectedId).subscribe((data) => {
        console.log(data);
  ...
  onClickSubmit(data: any) {
    ...
    let expenseEntry: ExpenseEntry = {
      ...
      createdOn: new Date(2022, 2, 14),
    };
    console.log(expenseEntry);

    if (expenseEntry.id == null || expenseEntry.id == 0) {
      console.log('add fn fired');
      this.entryService.addExpenseEntry(expenseEntry).subscribe((data) => {
        console.log(data);
        this.router.navigate(['/expenses']);
      });
    } else {
      console.log('edit fn fired');
      this.entryService.updateExpenseEntry(expenseEntry).subscribe((data) => {
        console.log(data);
        this.router.navigate['/expenses'];
      });
    }
